...
class DataLoader:
    ...
    # Features whose original data sources (Google PageRank API,
    # Alexa traffic rankings, paid backlink/SEO APIs) are discontinued
    # or require paid access as of 2026. Dropped from training so the
    # model only relies on features that can be reliably computed live.
    ...
    def load_uci_dataset(self, filepath: str) -> Tuple[pd.DataFrame, pd.Series]:
        ...
        5 features that depended on now-discontinued external services
        (Page_Rank, web_traffic, Google_Index, Statistical_report,
        Links_pointing_to_page) are dropped, leaving 25 usable features.
        ...
        # Drop features that rely on discontinued/paid external services
        columns_to_drop = [
            c for c in self.DISCONTINUED_FEATURES if c in X.columns]
        if columns_to_drop:
            print(f"Dropping discontinued-service features: {columns_to_drop}")
            X = X.drop(columns=columns_to_drop)

        # Convert UCI labels to 0/1:
        # UCI convention: b'1' = phishing, b'-1' = legitimate
        # We map: phishing → 1, legitimate → 0
        y = y.apply(lambda x: 0 if str(x).replace(
            "b'", "").replace("'", "").strip() == "-1" else 1)
    ...
    def load_custom_dataset(self, filepath: str,
        ...
        y = labels.apply(lambda x: 1 if str(x).lower() in [
                         'phishing', '1', 'malicious', 'bad', 'true'] else 0)
    ...
    def create_sample_dataset(self, output_path: str = 'data/sample_urls.csv',
        ...
        all_legitimate = legitimate_urls * \
            ((n_samples // 2) // len(legitimate_urls) + 1)
        all_phishing = phishing_patterns * \
            ((n_samples // 2) // len(phishing_patterns) + 1)
